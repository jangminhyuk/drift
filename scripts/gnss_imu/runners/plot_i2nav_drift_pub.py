"""Candidate publication figures for WRAP on i2Nav (v6_c, muv-only deploy).

Generates several distinct candidates so we can pick:
  A. aggregate RMSE decomposition (KF-GINS / DR-only / Adapter-only / WRAP)
  B. per-sequence dumbbell (baseline -> adapter -> WRAP), shows consistency
...
ROOT = Path(os.environ.get('WRAP_GNSS_DATA', str(Path(__file__).resolve().parents[3] / 'data/precomputed/gnss')))
TRAJ = ROOT / 'traj'
OUT = ROOT / 'figures' / 'pub_v1'

# semantic palette: baseline gray / DR blue / adapter red / WRAP violet
C_BASE = '#898781'
C_DR = '#2a78d6'
C_ADAP = '#e34948'
C_WRAP = '#4a3aa7'
...
def fig_agg_bars(S):
    ...
    labels = ['KF-GINS', 'DR only', 'Adapter only', 'WRAP']
    colors = [C_BASE, C_DR, C_ADAP, C_WRAP]
...
def fig_dumbbell(S):
    ...
    for i, (s, base, adap, drift) in enumerate(rows):
        ...
        ax.scatter(drift, i, s=95, color=C_WRAP, zorder=4, edgecolor='black', lw=0.5)
    ...
    h = [Line2D([0], [0], marker='o', color='w', mfc=C_BASE, mec='k', ms=10, label='KF-GINS'),
         ...
         Line2D([0], [0], marker='o', color='w', mfc=C_WRAP, mec='k', ms=10, label='WRAP')]
...
def fig_error_time(seq, smooth_s=4.0):
    ...
    ax.plot(td - t0, rd, color=C_WRAP, lw=1.9, label='WRAP', zorder=4)
...
def fig_trajectory(seq):
    """Top-down NED trajectory: GT vs KF-GINS vs WRAP. Dramatic only when the
    ...
    ax.plot(ed[:, 1], ed[:, 0], color=C_WRAP, lw=1.6, alpha=0.95,
            label='WRAP', zorder=3)
...
def fig_tail_dumbbell():
    ...
    adapter -> WRAP. Honestly shows building01's tail REGRESSION."""
    def p95(s, m):
...
def fig_per_seq_error_time(S, smooth_s=4.0):
    ...
    for k, s in enumerate(SEQS):
        ...
        ax.plot(td - t0, rd, color=C_WRAP, lw=1.7, zorder=4)
        red = 100 * (ct[s]['baseline']['rmse_h'] - ct[s]['adapter_dr']['rmse_h']) \
            / ct[s]['baseline']['rmse_h']
        ax.set_title(f'{s}   (WRAP $-${red:.0f}%)', fontsize=12.5)
        ax.set_xlim(0, (tb - t0)[-1])
    ...
    h = [Line2D([0], [0], color=C_BASE, lw=2.4, label='KF-GINS'),
         ...
         Line2D([0], [0], color=C_WRAP, lw=2.6, label='WRAP')]
    axes[7].axis('off')
...
def fig_per_seq_improvement(S):
    ...
    for off, vals, c in [(-w, dr, C_DR), (0, adap, C_ADAP), (w, drift, C_WRAP)]:
    ...
    handles = [Patch(fc=to_rgba(C_DR, 0.35), ec=C_DR, lw=1.6, label='DR-only'),
               ...
               Patch(fc=to_rgba(C_WRAP, 0.35), ec=C_WRAP, lw=1.6, label='WRAP')]
